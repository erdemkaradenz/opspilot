...
class Organization(Base):
    ...
    created_at: Mapped[datetime] = mapped_column(
        DateTime, default=datetime.utcnow, server_default=sa.func.now()
    )
...
class User(TenantAwareModel):
    ...
    __table_args__ = (
        ...
        ),  # 10 bin kullanıcıda veritabanının kilitlenmemesi için index!
    )
...
class Service(TenantAwareModel):
    ...
    __table_args__ = (Index("ix_services_org_id", "org_id"),)
...
class Incident(TenantAwareModel):
    ...
    severity: Mapped[str] = mapped_column(
        SQLEnum(IncidentSeverity), default=IncidentSeverity.SEV_3, nullable=False
    )
