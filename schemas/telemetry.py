...
class TelemetryEvent(BaseModel):
    tenant_id: UUID = Field(
        ..., description="Müşterinin (Organizasyon) benzersiz kimliği"
    )
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="Olayın gerçekleşme zamanı"
    )
    fingerprint: str = Field(
        ...,
        description="Olayı tekilleştirmek için benzersiz parmak izi (örn: redis_timeout_v1)",
    )
