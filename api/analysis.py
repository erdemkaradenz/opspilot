...
async def trigger_ai_analysis(
    ...
    if not incident:
        raise HTTPException(
            status_code=404, detail=f"Incident bulunamadı: {incident_id}"
        )
    ...
    task_payload = {"job_id": job_id, "incident_id": str(incident_id)}

    # Gerçek sistemlerde tracing (Correlation ID) buraya da enjekte edilir.
    await producer.send_and_wait(
        "ai_analysis_tasks", json.dumps(task_payload).encode("utf-8")
    )
