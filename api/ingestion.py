...
async def custom_rate_limiter(request: Request):
    ...
    if requests > 5:
        raise HTTPException(status_code=429, detail="Too Many Requests - OpsPilot Shield")

@router.post(
    "/telemetry/events", 
    status_code=202, 
    dependencies=[Depends(custom_rate_limiter)] # Kendi yazdığımız koruma kalkanı devrede
)
async def ingest_telemetry(
    ...
    return JSONResponse(
        status_code=202, 
        content={"status": "Accepted", "message": "Event queued for processing", "fingerprint": event.fingerprint}
    )