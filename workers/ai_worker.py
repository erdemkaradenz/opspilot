...
async def process_ai_analysis(task_data: dict):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not evidences:
            print(
                "[AI WORKER] Evidence bulunamadı! AI analizi atlanıyor. Kural bazlı bildirim gönderiliyor."
            )
        ...
        rag_stmt = (
            ...
            .order_by(Incident.embedding.cosine_distance(current_embedding))
            .limit(3)
        )
        ...
        context_text = "\n".join(
            [
                f"- Eski Olay ({sim.id}): {sim.ai_rca_summary}"
